...
class UserModelCase(unittest.TestCase):
    ...
    def test_password_hashing(self):
        u = User(username='usan')
        u.set_pw('cat')
        self.assertFalse(u.check_pw('dog'))
        self.assertTrue(u.check_pw('cat'))
    ...
    def test_follow(self):
        u1 = User(username='tae', email='tae@example.com')
        u2 = User(username='rachel', email='rachel@example.com')
        db.session.add(u1)
        db.session.add(u2)
        db.session.commit()
        ...
        self.assertEqual(u2.followers.first().username, 'tae')
        # Additional
